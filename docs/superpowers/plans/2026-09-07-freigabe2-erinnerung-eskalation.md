...
**Goal:** A job stuck in `freigabe2` (Freigabe 1 already granted) with an inactive responsible person now gets a personal reminder mail after a configurable number of hours, and is automatically handed to the Admin group once a second, independently configurable number of hours has elapsed since the same starting point (not added on top of the reminder threshold) — mirroring the existing `pool-erinnerungen` mechanism but scoped to `freigabe2` and its actually-responsible person.
