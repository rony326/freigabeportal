...
- Neue `admin_config`-Einträge (Default jeweils leerer String, ausser vermerkt):
  ...
  - `cron_zeitstempel_nachholen_intervall_minuten` (Default: `5`) — bewusst kurz, siehe Abschnitt "Bekannte Einschränkung" unten.
...
## Bekannte Einschränkung: Zeitfenster vor n8n-Abholung

Sobald n8n einen Job abholt (`POST /api/n8n/jobs/:id/abholung-bestaetigen`), löscht das Portal die lokale PDF-Datei (`src/routes/n8n/jobs.js`). Ein Zeitstempel kann danach nicht mehr nachträglich gesetzt werden — die Datei existiert nicht mehr. Das Nachhol-Intervall (Standard 5 Minuten) ist bewusst kurz gewählt, um das Zeitfenster vor einer typischen n8n-Abholung realistisch zu treffen, aber ein Restrisiko bleibt: fällt die TSA exakt beim Freigabe-2-Abschluss aus und holt n8n den Job sehr schnell danach ab, bleibt dieser eine Job dauerhaft ohne Zeitstempel. Akzeptiert, da "nicht blockierend" explizit gewählt wurde. Die Dashboard-Sektion (siehe unten) macht diesen Zustand für Betroffene sichtbar.
...
Neue eigenständige Admin-Seite `views/admin/zeitstempel-form.ejs` + `src/routes/admin/zeitstempel.js`, gemountet unter `/admin/zeitstempel` (Portal-Admin-Rolle, wie die übrigen `/admin/*`-Seiten) — folgt exakt dem Formular-Muster von `erscheinungsbild.js` (GET/POST, Validierung, `?gespeichert=1`-Flash). Felder: TSA-URL (Pflicht für aktives Feature, sonst leer lassen zum Deaktivieren), Benutzername (optional), Passwort (optional, wie ein normales Passwortfeld — kein Klartext-Reveal). Neuer Eintrag im Admin-Dashboard (`views/admin/dashboard.ejs`) und in der Admin-Nav (`views/admin/_nav.ejs`).
...
- Integrationstest für `/admin/zeitstempel` (401/403/Validierung/Speichern), nach dem Muster von `erscheinungsbild.test.js`.
...
- Keine Rückwirkende Zeitstempelung bereits `abgeholt`/`archiviert`er Jobs — technisch unmöglich, da die PDF-Datei nicht mehr existiert (siehe "Bekannte Einschränkung").
...
**Geändert:**
- `src/db/schema.sql`, `src/db/index.js` (Migration: `jobs.zeitstempel_gesetzt_am`)
- `src/db/adminConfigRepo.js` (neue DEFAULTS)
- `src/db/jobsRepo.js` (neue `listAbgeschlossenJobsForPerson`)
...
- `views/admin/dashboard.ejs`, `views/admin/_nav.ejs` (neue Admin-Seite verlinken)
